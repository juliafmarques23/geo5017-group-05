...
def best_models_rf(X, y):
    hyperparams = {
        'n_estimators': [50, 100, 200], 'criterion': ['gini', 'entropy'],
        'max_depth': [5, 10, 20], 'min_samples_split': [2, 5],
        'min_samples_leaf': [1, 2, 5], 'max_features': [2, 4],
        'max_leaf_nodes': [200, 400], 'bootstrap': [True], 'max_samples': [0.5, 1.0]
    }

    rfc = RandomForestClassifier()
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.2)

    grid = GridSearchCV(rfc, hyperparams, cv=5, return_train_score=True)
    grid.fit(X_train, y_train)
    results = pd.DataFrame(grid.cv_results_)

    best_models = results.sort_values(by=['rank_test_score']).head(10).copy()
    best_models['diff'] = abs(best_models['mean_train_score'] - best_models['mean_test_score'])
    cols_to_keep = ['params', 'mean_train_score', 'mean_test_score', 'diff', 'rank_test_score']
    final = best_models[cols_to_keep]
    final.to_csv("best_models.csv", header=True, index=False)
...
def learning_curve(X, y):
    ...
    for i in range(5, 100, 5):
        X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=i*0.01, random_state=42)
        rfc.fit(X_train, y_train)
        ...
        test_samples.append(i*0.01)

    fig = plt.figure()
    plt.plot(test_samples, train_scores, label='Train score', marker='o')
    plt.plot(test_samples, test_scores, label='Test Score', marker='o')
    plt.ylim(0.8, 1.0)
...
if __name__=='__main__':
    ...
    X, y = filtered_outputs[:, 2:-1], filtered_outputs[:, 1]
